...
//     {
//         color(12);
//         cout << "No accounts to add!" << endl;
//         color(15);
//         return;
//     }
//     Customer *tempCustomer = new Customer[add];
//     cin.ignore();
//     for (int i = 0; i < add; i++)
//     {
//         cout << "Customer " << i + 1 << endl;
//         tempCustomer[i].inputData();
//         cout << endl;
//     }
//     fout.open(database, ios::app | ios::ate | ios::out);
//     if (fout.is_open())
//     {
//         for (int i = 0; i < add; i++)
//         {
...
//     tempCustomer = NULL;
//     delete[] tempCustomer;
//     readDatabase(customer, n, accNumChk);
// }
void Customer::modifyAccount(unsigned int *n = n)
{
    ...
    cin >> tempId;
    tempId = idChk(n, tempId);
    trav = head;
    for (int i = 0; i < tempId; i++)
    {
        trav = trav->next;
    }
    if (tempId == -1 || tempId >= *accNumChk)
    {
        accountFailure();
        return;
    }
...
// void alotCash(Customer *customer = customer, unsigned int *n = n)
// {
//     unsigned int amount = 0;
//     unsigned int tempId = 0;
//     cout << "Enter the ID of the account : ";
//     cin >> tempId;
//     tempId = idChk(customer, n, tempId);
//     if (tempId == -1)
//     {
//         accountFailure();
//         return;
...
//     customer[tempId].outputInfo();
//     cout << endl;
//     cout << "Enter amount to alot : ";
//     cin >> amount;
//     if (amount > 0)
//     {
//         customer[tempId].receiveCash(amount);
//         transactionSuccess();
//         string statFile = Statement(customer[tempId].getId());
...
//     }
// }
// void alotLoan(Customer *customer = customer, unsigned int *n = n)
// {
//     unsigned int amount = 0;
//     unsigned int tempId = 0;
//     cout << "Enter the ID of the account : ";
...
//     {
//         customer[tempId].receiveCash(amount);
//         customer[tempId].addLoan(amount);
//         transactionSuccess();
//         string statFile = Statement(customer[tempId].getId());
//         fout.open(statFile, ios::app | ios::out);
//         if (fout.is_open())
...
//     }
// }
// void outputDatabase(Customer *customer = customer, unsigned int *n = n)
// {
//     cout << "FirstName \t LastName \t AccID \t\t AccType \t Balance \t Loan" << endl;
//     cout << endl;
//     for (int i = 0; i < *n; i++)
//     {
//         customer[i].outputInfo();
//     }
// }
//----------------------------------------------------------------
...
//     cin >> receiverId;
//     receiverId = customer->idChk(n, receiverId);
//     if (receiverId == -1)
//     {
//         accountFailure();
//         return;
...
//         return;
//     }
//     cout << "Enter amount to sent to " << customer[receiverId].getFirstName() << " " << customer[receiverId].getLastName() << " : ";
//     cin >> amount;
//     limit = accTypeChk();
//     if (amount <= accBalance && amount > 0 && amount <= limit)
//     {
...
//         transactionSuccess();
//         cout << "Sent " << amount << " to " << customer[receiverId].getFirstName() << " " << customer[receiverId].getLastName() << endl;
//         accBalance -= amount;
//         customer[receiverId].receiveCash(amount);
//         cout << "Remaining Balance : " << accBalance << endl;
//         updateDatabase();
//         string statFile = Statement(accId);
//         fout.open(statFile, ios::app | ios::out);
//         if (fout.is_open())
//         {
//             fout << "Sent - " << amount << " to " << customer[receiverId].getFirstName() << " " << customer[receiverId].getLastName() << " " << customer[receiverId].getId() << endl;
//         }
//         else
//         {
//             statementFailure();
//         }
//         fout.close();
//         statFile = Statement(customer[receiverId].getId());
//         fout.open(statFile, ios::app | ios::out);
//         if (fout.is_open())
//         {
...
bool Customer::customerLogin(unsigned int *n = n)
{
    ...
    *id = tempId;
    ...
    acc = customer->searchAccountNode(*id);

    if (intPass == acc->accPin && acc != NULL)
    {
        color(10);
        g_xy(52, 21);
